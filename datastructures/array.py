...
class Array(IArray[T]):  
    ...
    def __init__(self, starting_sequence: Sequence[T]=[], data_type: type=object) -> None: 
        ...
        self.__physcial_size: int = self.__logical_size
        self.__data_type: int = data_type
        ...
        for index in range(self.__logical_size):
            if not isinstance(starting_sequence[index], self.__data_type):
                raise TypeError('Items in starting are not all the same type')

        self._items: NDArray = np.empty(self.__logical_size, dtype=self.__data_type)
